...
class Program:
    ...
    def KhBouwerRice_FP(self):
        """Bouwer-Rice calculations for unconfined aquifer, fully penetrating well
        The method is documented in Butler, 2019, The Design, Performance, and Analysis
        of Slug Tests. References to equations for each variable in comments below."""
        ...
        # C is given as equation 6.5c
        self.C = 0.7920 + \
            0.03993*(self.Le/self.rw_as) - \
            0.00005743*(self.Le/self.rw_as)**2 + \
            0.00000003858*(self.Le/self.rw_as)**3 - \
            0.000000000009659*(self.Le/self.rw_as)**4

        # equation 6.4b
        self.logterm_BR_FP = (1.1 / math.log((self.d+self.Le) / self.rw_as) +
                        (self.C / (self.Le / self.rw_as)))**(-1)

        # equation 6.3
        self.Kh_BR_FP = round((self.rc**2 * self.logterm_BR_FP) / (2*self.Le * self.T01),
                              5)  # rounding off to five decimals

    def KhHvorslev_PP(self):
        """Hvorslev calculations for confined aquifer, partially penetrating well"""
        self.F = 1 / (2*math.sqrt(self.Aniso) / (self.Le/self.rw)) + \
            math.sqrt(1 + (1/(2*math.sqrt(self.Aniso)/(self.Le/self.rw)))**2)

        # equation 5.19
        self.Kh_H_PP = round(
            (self.rc**2 * math.log(self.F)) / (2*self.Le*self.T01),
            5)  # rounding off to five decimals

    def KhHvorslev_FP(self):
        """Hvorslev calculations for confined aquifer, fully penetrating well"""
        self.Kh_H_FP = round(
            (self.rc**2 * math.log(self.re/self.rw)) / (2*self.AqThick*self.T01),
            5)  # rounding off to five decimals
    ...
    def PerformCalc(self):
        try:
            ...
            self.d = float(D.get()) # z-position of end of screen closest to an impermeable boundary in confined
                                    # formation or to water table in unconfined formation [L]
            self.Le = float(LE.get())  # effective screen length [L]
            self.rc = float(RC.get())  # effective radius well casing [L]
            self.rw = float(RW.get())  # effective radius well screen [L]
            ...
            if V7.get()==True:
                self.AqThick=float(AQTHICK.get())
                self.re = float(RE.get())  # effective radius parameter [L]
